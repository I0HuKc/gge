...
// Метод загрузки PNG изображений
  ...
  *height = t_height;
  ...
  switch (png_get_color_type(png_ptr, info_ptr)) {
    ...
    case PNG_COLOR_TYPE_RGB:
      ...
      break;
    default:
      printf("Цвет с типом %d не поддерживается!\n",
             png_get_color_type(png_ptr, info_ptr));
      png_destroy_read_struct(&png_ptr, &info_ptr, &end_info);
      return 0;
  }

  // генерация текстуры и получение ее id
  glGenTextures(1, &texture);
  // привязка текстуры
  glBindTexture(GL_TEXTURE_2D, texture);
  glPixelStorei(GL_UNPACK_ALIGNMENT, 1);

  // передача текстурных данных в текстуру
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, t_width, t_height, 0, alpha,
               GL_UNSIGNED_BYTE, (GLvoid*)image_data);

  // установка текстурных данных
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);

  // отвязываю текстуру после загрузки
  glBindTexture(GL_TEXTURE_2D, 0);

  // освобождение и удаление
  png_destroy_read_struct(&png_ptr, &info_ptr, &end_info);
  free(image_data);
  free(row_pointers);
  fclose(f);
...
Texture* LoadTexture(std::string filename) {
  ...
  if (texture == 0) {
    std::cerr << "Не удалось найти текстуру с именем  " << filename
              << std::endl;
    return nullptr;
  }
